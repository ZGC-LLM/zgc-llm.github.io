...
当前仍需外部完成正式域名、三个 Feishu 表单匿名流程，以及适用的备案与公开联系渠道。main 暂不要求批准者，但 PR、三个 strict checks、对话解决与管理员保护仍为强制门禁。正式 Logo 和官方英文全称未确认，本地候选公开输出已分别使用中性文字/程序化图形和 `ZGCLLM` / `the Alliance`；只有恢复正式资产或全称时才须先完成权利人确认。GitHub Pages 实际端点可能仍服务较旧提交，候选源码通过不等于现网已更新；候选 SHA、现网 SHA、外部门和发布裁决以 [发布就绪报告](./docs/dev/repository-wide-release-optimization/release-readiness-report.md) 为准，本 README 不代替发布报告。
